...
change_puzzle(Puzzle, [], Solved),
findAllSlots(NewPuzzle, [], AllSLots),
put_into_slots(AllSLots, WordList)


change_puzzle([], A, A).
change_puzzle([Row|OtherRows], A, NewPuzzle) :-
change_row(Row, [], NewRow),
append(A, [NewRow], A1),
change_puzzle(OtherRows, A1, NewPuzzle).

change_row([], A, A).
change_row([Head|Tail], A, NewRow) :-
(Head == '_' ->
append((A, [Slot], A1)
; append(A, [Head], A1)
),
